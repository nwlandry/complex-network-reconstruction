...
def collect_parameters(dir):
    ...
    a_dict = {a: i for i, a in enumerate(sorted(alist))}
    r_dict = {r: i for i, r in enumerate(sorted(rlist))}

    return c_dict, b_dict, a_dict, r_dict


def get_metrics(f, dir, c_dict, b_dict, a_dict, r_dict):
...
c_dict, b_dict, a_dict, r_dict = collect_parameters(data_dir)
